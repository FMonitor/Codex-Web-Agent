...
export class CodexCliAdapter implements RuntimeAdapter {
  ...
  private readonly proxyPort = process.env.CODEX_RESPONSES_PROXY_PORT?.trim() || "11434";
  ...
  private getProfileBaseUrl(profile: string): string | null {
    if (profile === "custom-api") {
      return process.env.CODEX_CUSTOM_API_BASE_URL?.trim() || `http://127.0.0.1:${this.proxyPort}/v1`;
    }
